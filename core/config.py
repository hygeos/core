...
def get_config_file(mode: Literal["global", "local", "auto"]) -> Path:
    ...
    local_config_file_old = Path.cwd() / 'config.toml'
    if local_config_file_old.exists():
        with open(local_config_file_old) as fp:
            lines = fp.readlines()
        if 'This file has been generated by eoread.utils.config' in ''.join(lines):
            warn(f"core.config: {local_config_file_old} is an "
